...
def dataset_statistics(dataset_path: str):
    ...
    # Label distribution
    label_counts_train = train_dataloader.y.to_frame().groupby(DEFAULT_LABELS_COL).count().to_dict(as_series=False)
    label_counts_test = train_dataloader.y.to_frame().groupby(DEFAULT_LABELS_COL).count().to_dict(as_series=False)
